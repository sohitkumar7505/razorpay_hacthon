...
function MerchantControls() {
  ...
  return <section className="mb-7 grid gap-6 lg:grid-cols-2">
    <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5"><p className="text-xs font-black tracking-widest text-amber-300">CATALOGUE MANAGEMENT</p><h2 className="mt-2 text-xl font-black">Products and inventory</h2><label className="mt-3 block cursor-pointer rounded-lg border border-dashed border-amber-300/30 px-3 py-2 text-center text-xs font-bold text-amber-200">Import catalogue CSV<input type="file" accept=".csv,text/csv" onChange={importCsv} className="sr-only" /></label><div className="mt-4 max-h-48 space-y-2 overflow-auto">{products.map((product) => <div key={product.id} className="flex items-center justify-between gap-2 rounded-lg bg-black/20 px-3 py-2 text-sm"><span>{product.name}</span><span className="ml-auto text-slate-400">{formatMoney(product.price)} · {product.inventory} stock</span><button onClick={() => setDraft({ ...product, tags: product.tags.join(","), complements: product.complements.join(",") })} className="text-xs font-bold text-blue-300">Edit</button><button onClick={() => deleteProduct(product.id)} className="text-xs font-bold text-rose-300">Delete</button></div>)}</div><form onSubmit={saveProduct} className="mt-4 grid gap-2 sm:grid-cols-2">{["id","name","description","category","price","inventory","shippingDays","tags","complements"].map((field) => <input key={field} required={["id","name","description","category"].includes(field)} type={["price","inventory","shippingDays"].includes(field) ? "number" : "text"} value={draft[field]} onChange={(event) => setDraft((current) => ({ ...current, [field]: event.target.value }))} placeholder={field} className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm" />)}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={draft.returnable} onChange={(event) => setDraft((current) => ({ ...current, returnable: event.target.checked }))} /> Returnable</label><button className="rounded-lg bg-amber-300 px-3 py-2 font-black text-amber-950">Add or update product</button></form></div>
